...
mutable struct Texture{T<:GLArrayEltypes,NDIM} <: OpenglTexture{T,NDIM}
    ...
    context::GLContext
    function Texture{T,NDIM}(id::GLuint, texturetype::GLenum, pixeltype::GLenum, internalformat::GLenum,
                             format::GLenum, parameters::TextureParameters{NDIM},
                             size::NTuple{NDIM,Int}) where {T,NDIM}
        tex = new(id, texturetype, pixeltype, internalformat, format, parameters, size, current_context())
        finalizer(free, tex)
        return tex
    end
end
...
function Texture(data::Ptr{T}, dims::NTuple{NDim,Int}; internalformat::GLenum=default_internalcolorformat(T),
                 texturetype::GLenum=default_texturetype(NDim), format::GLenum=default_colorformat(T),
                 mipmap=false, parameters...) where {T,NDim}
    ...
    texture = Texture{T,NDim}(id, texturetype, numbertype, internalformat, format, texparams, dims)
...
"""
function Texture(::Type{T}, dims::NTuple{N,Int}; kw_args...) where {T<:GLArrayEltypes,N}
    return Texture(convert(Ptr{T}, C_NULL), dims; kw_args...)::Texture{T,N}
end
...
"""
function Texture(image::Array{T,NDim}; kw_args...) where {T<:GLArrayEltypes,NDim}
    return Texture(pointer(image), size(image); kw_args...)::Texture{T,NDim}
end

function Texture(s::ShaderAbstractions.Sampler{T,N}; kwargs...) where {T,N}
    tex = Texture(pointer(s.data), size(s.data), minfilter=s.minfilter, magfilter=s.magfilter,
...
"""
function Texture(data::Vector{Array{T,2}}; internalformat::GLenum=default_internalcolorformat(T),
                 texturetype::GLenum=GL_TEXTURE_2D_ARRAY, format::GLenum=default_colorformat(T),
                 parameters...) where {T<:GLArrayEltypes}
    ...
    texture = Texture{T,2}(id, texturetype, numbertype, internalformat, format, texparams, tuple(maxdims...))
    set_parameters(texture)
    return texture
end
...
function TextureBuffer(buffer::GLBuffer{T}) where {T<:GLArrayEltypes}
    ...
    tex = Texture{T,1}(id, texture_type, julia2glenum(T), internalformat, default_colorformat(T),
                       TextureParameters(T, 1), size(buffer))
...
# GPUArray interface:
function unsafe_copy!(a::Vector{T}, readoffset::Int, b::TextureBuffer{T}, writeoffset::Int,
                      len::Int) where {T}
...
function unsafe_copy!(a::TextureBuffer{T}, readoffset::Int, b::Vector{T}, writeoffset::Int,
                      len::Int) where {T}
...
function unsafe_copy!(a::TextureBuffer{T}, readoffset::Int, b::TextureBuffer{T}, writeoffset::Int,
                      len::Int) where {T}
    ...
    return glBindTexture(t.texture.texturetype, 0)
end
...
function gpu_setindex!(t::Texture{T,1}, newvalue::Array{T,1}, indexes::UnitRange{I}) where {T,I<:Integer}
    ...
    return glBindTexture(t.texturetype, 0)
end
function gpu_setindex!(t::Texture{T,N}, newvalue::Array{T,N},
                       indexes::Union{UnitRange,Integer}...) where {T,N}
    glBindTexture(t.texturetype, t.id)
    texsubimage(t, newvalue, indexes...)
    return glBindTexture(t.texturetype, 0)
end
...
function gpu_setindex!(target::Texture{T,2}, source::Texture{T,2}, fbo=glGenFramebuffers()) where {T}
    ...
    glFramebufferTexture2D(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, source.id, 0)
    glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, target.id, 0)
    glDrawBuffer(GL_COLOR_ATTACHMENT1)
    w, h = map(minimum, zip(size(target), size(source)))
    return glBlitFramebuffer(0, 0, w, h, 0, 0, w, h, GL_COLOR_BUFFER_BIT, GL_NEAREST)
end
...
function similar(t::Texture{T,NDim}, newdims::NTuple{NDim,Int}) where {T,NDim}
    return Texture(Ptr{T}(C_NULL), newdims, t.texturetype, t.pixeltype, t.internalformat, t.format,
                   t.parameters)
end
# Resize Texture
...
function texsubimage(t::Texture{T,1}, newvalue::Array{T,1}, xrange::UnitRange, level=0) where {T}
    return glTexSubImage1D(t.texturetype, level, first(xrange) - 1, length(xrange), t.format, t.pixeltype,
                           newvalue)
end
function texsubimage(t::Texture{T,2}, newvalue::Array{T,2}, xrange::UnitRange, yrange::UnitRange,
                     level=0) where {T}
    return glTexSubImage2D(t.texturetype, level, first(xrange) - 1, first(yrange) - 1, length(xrange),
                           length(yrange), t.format, t.pixeltype, newvalue)
end
function texsubimage(t::Texture{T,3}, newvalue::Array{T,3}, xrange::UnitRange, yrange::UnitRange,
                     zrange::UnitRange, level=0) where {T}
    return glTexSubImage3D(t.texturetype, level, first(xrange) - 1, first(yrange) - 1, first(zrange) - 1,
                           length(xrange), length(yrange), length(zrange), t.format, t.pixeltype, newvalue)
end
...
function default_colorformat_sym(::Type{T}) where {T<:AbstractArray}
    return default_colorformat_sym(cardinality(T), eltype(T) <: Integer, "RGBA")
end
function default_colorformat_sym(::Type{T}) where {T<:StaticVector}
    return default_colorformat_sym(cardinality(T), eltype(T) <: Integer, "RGBA")
end
function default_colorformat_sym(::Type{T}) where {T<:Colorant}
    return default_colorformat_sym(cardinality(T), eltype(T) <: Integer, string(Base.typename(T).name))
end
...
function default_texturetype(ndim::Integer)
    ...
    return error("Dimensionality: $(ndim), not supported for OpenGL texture")
end
...
function TextureParameters(T, NDim; minfilter=T <: Integer ? :nearest : :linear, magfilter=minfilter, # magnification # magnification # magnification # magnification # magnification # magnification # magnification # magnification
    ...
    T <: Integer &&
        ...
        error("Wrong Texture Parameter: Integer texture can't interpolate. Try :nearest")
    ...
    return TextureParameters(minfilter, magfilter, ntuple(i -> repeat[i], NDim), anisotropic, swizzle_mask)
end
