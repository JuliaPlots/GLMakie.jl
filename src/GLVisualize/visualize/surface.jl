...
function _default(main::Tuple{MatTypes{T},MatTypes{T},MatTypes{T}}, s::Style{:surface},
                  data::Dict) where {T<:AbstractFloat}
    ...
    return surface(position_z, s, data)
end

# surface(Vector or Range, Vector or Range, ::Matrix)
function _default(main::Tuple{VectorTypes{T},VectorTypes{T},MatTypes{T}}, s::Style{:surface},
                  data::Dict) where {T<:AbstractFloat}
    @gen_defaults! data begin
...
function _default(main::MatTypes{T}, s::Style{:surface}, data::Dict) where {T<:AbstractFloat}
    @gen_defaults! data begin
        ranges = ((-1.0f0, 1.0f0), (-1.0f0, 1.0f0)) => "x, and y position given as `(start, endvalue)` or any `Range`"
    end
    delete!(data, :ranges) # no need to have them in the OpenGL data
    return _default((Grid(to_value(main), to_value(ranges)), main), s, data)
end

# surface(::Matrix)
function _default(main::Tuple{G,MatTypes{T}}, s::Style{:surface},
                  data::Dict) where {G<:Grid{2},T<:AbstractFloat}
    xrange = main[1].dims[1]
    yrange = main[1].dims[2]
...
function surface(main, s::Style{:surface}, data::Dict)
    @gen_defaults! data begin
        primitive = Rect2D(0.0f0, 0.0f0, 1.0f0, 1.0f0) => native_triangle_mesh
        scale = nothing
        position = nothing
        ...
        stroke_width = wireframe ? 0.03f0 : 0.0f0
        glow_width = 0.0f0
    ...
    @gen_defaults! data begin
        ...
        shader = GLVisualizeShader("fragment_output.frag", "util.vert", "surface.vert",
                                   to_value(wireframe) ? "distance_shape.frag" : "standard.frag",
                                   view=Dict("position_calc" => position_calc(position, position_x,
                                                                              position_y, position_z, Texture),
...
function _position_calc(position_x::MatTypes{T}, position_y::MatTypes{T}, position_z::MatTypes{T},
                        target::Type{Texture}) where {T<:AbstractFloat}
    return """
...
function _position_calc(position_x::VectorTypes{T}, position_y::VectorTypes{T}, position_z::MatTypes{T},
                        target::Type{Texture}) where {T<:AbstractFloat}
    return """
           int index1D = index + offseti.x + offseti.y * dims.x + (index/(dims.x-1));
           ivec2 index2D = ind2sub(dims, index1D);
...
function _position_calc(position_xyz::VectorTypes{T}, target::Type{TextureBuffer}) where {T<:StaticVector}
    return "pos = texelFetch(position, index).xyz;"
end

function _position_calc(position_xyz::VectorTypes{T}, target::Type{GLBuffer}) where {T<:StaticVector}
    len = length(T)
...
function _position_calc(grid::Grid{2}, position_z::MatTypes{T},
                        target::Type{Texture}) where {T<:AbstractFloat}
    return """
           int index1D = index + offseti.x + offseti.y * dims.x + (index/(dims.x-1));
           ivec2 index2D = ind2sub(dims, index1D);
