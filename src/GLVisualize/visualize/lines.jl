...
function ticks(points, resolution)
    return Float16[gappy(x, points) for x in range(first(points), stop=last(points), length=resolution)]
end

# ambigious signature
function _default(position::VectorTypes{<:Point}, s::style"lines", data::Dict)
    return line_visualization(position, data)
end
...
function line_visualization(position::Union{VectorTypes{T},MatTypes{T}}, data::Dict) where {T<:Point}
    ...
    @gen_defaults! data begin
        ...
        shader = GLVisualizeShader("fragment_output.frag", "util.vert", "lines.vert", "lines.geom",
                                   "lines.frag")
        gl_primitive = GL_LINE_STRIP_ADJACENCY
        valid_vertex = const_lift(p_vec) do pv
            return map(p -> Float32(all(isfinite, p)), pv)
        end => GLBuffer
    end
    if pattern != nothing
...
function _default(positions::VectorTypes{T}, s::style"linesegment", data::Dict) where {T<:Point}
    @gen_defaults! data begin
        ...
        shader = GLVisualizeShader("fragment_output.frag", "util.vert", "line_segment.vert",
