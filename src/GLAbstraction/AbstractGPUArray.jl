...
function checkdimensions(value::Array, ranges::Union{Integer,UnitRange}...)
    ...
    (array_size != indexes_size) &&
        throw(DimensionMismatch("asigning a $array_size to a $(indexes_size) location"))
    return true
end
function to_range(index)
    map(index) do val
        ...
        return error("Indexing only defined for integers or ranges. Found: $val")
    end
end
...
function copy!(a::GPUVector, a_offset::Int, b::Vector, b_offset::Int, amount::Int)
    return copy!(a.buffer, a_offset, b, b_offset, amount)
end
function copy!(a::GPUVector, a_offset::Int, b::GPUVector, b_offset::Int, amount::Int)
    return copy!(a.buffer, a_offset, b.buffer, b_offset, amount)
end

function copy!(a::GPUArray, a_offset::Int, b::Vector, b_offset::Int, amount::Int)
    return _copy!(a, a_offset, b, b_offset, amount)
end
function copy!(a::Vector, a_offset::Int, b::GPUArray, b_offset::Int, amount::Int)
    return _copy!(a, a_offset, b, b_offset, amount)
end
function copy!(a::GPUArray, a_offset::Int, b::GPUArray, b_offset::Int, amount::Int)
    return _copy!(a, a_offset, b, b_offset, amount)
end
...
# Interface:
function gpu_data(t)
    return error("gpu_data not implemented for: $(typeof(t)). This happens, when you call data on an array, without implementing the GPUArray interface")
end
function gpu_resize!(t)
    return error("gpu_resize! not implemented for: $(typeof(t)). This happens, when you call resize! on an array, without implementing the GPUArray interface")
end
function gpu_getindex(t)
    return error("gpu_getindex not implemented for: $(typeof(t)). This happens, when you call getindex on an array, without implementing the GPUArray interface")
end
function gpu_setindex!(t)
    return error("gpu_setindex! not implemented for: $(typeof(t)). This happens, when you call setindex! on an array, without implementing the GPUArray interface")
end
function max_dim(t)
    return error("max_dim not implemented for: $(typeof(t)). This happens, when you call setindex! on an array, without implementing the GPUArray interface")
end
...
function Serialization.serialize(s::BaseSerializer, t::T) where {T<:GPUArray}
    ...
    return Serialization.serialize(s, Array(t))
end
