...
function _default(p::Tuple{TOrSignal{Pr},VectorTypes{P}}, s::Style,
                  data::Dict) where {Pr<:Primitives3D,P<:Point}
...
function vec2quaternion(r::StaticVector{2})
    return vec2quaternion(Vec3f0(r[1], r[2], 0))
end
function vec2quaternion(rotation::StaticVector{3})
    return AbstractPlotting.rotation_between(Vec3f0(0, 0, 1), Vec3f0(rotation))
end
...
function meshparticle(p, s, data)
    ...
    @gen_defaults! data begin
        ...
        shading = true
        shader = GLVisualizeShader("util.vert", "particles.vert", "fragment_output.frag", "standard.frag",
...
primitive_offset(x, scale) = const_lift(/, scale, -2.0f0)  # default offset
...
function _default(p::Tuple{TOrSignal{Matrix{C}},VectorTypes{P}}, s::Style,
                  data::Dict) where {C<:Colorant,P<:Point}
    ...
    return sprites(p, s, data)
end
function _default(p::Tuple{TOrSignal{Matrix{C}},VectorTypes{P}}, s::Style,
                  data::Dict) where {C<:AbstractFloat,P<:Point}
...
function _default(p::Tuple{VectorTypes{Matrix{C}},VectorTypes{P}}, s::Style,
                  data::Dict) where {C<:Colorant,P<:Point}
    images = to_value(p[1])
...
function _default(p::Tuple{TOrSignal{Pr},VectorTypes{P}}, s::Style, data::Dict) where {Pr<:Sprites,P<:Point}
    return sprites(p, s, data)
end

function _default(p::Tuple{TOrSignal{Pr},G}, s::Style, data::Dict) where {Pr<:Sprites,G<:Tuple}
    @gen_defaults! data begin
        shape = const_lift(primitive_shape, p[1])
        position = nothing => GLBuffer
        position_x = p[2][1] => GLBuffer
        position_y = p[2][2] => GLBuffer
        position_z = length(p[2]) > 2 ? p[2][3] : 0.0f0 => GLBuffer
    end
    return sprites(p, s, data)
end
...
function sprites(p, s, data)
    ...
    @gen_defaults! data begin
        ...
        fxaa = false
        shader = GLVisualizeShader("fragment_output.frag", "util.vert", "sprites.geom", "sprites.vert",
                                   "distance_shape.frag",
                                   view=Dict("position_calc" => position_calc(position, position_x,
                                                                              position_y, position_z,
                                                                              GLBuffer)))
...
function _default(main::TOrSignal{S}, s::Style, data::Dict) where {S<:AbstractString}
    @gen_defaults! data begin
        ...
        uv_offset_width = const_lift(main) do str
            return Vec4f0[glyph_uv_width!(atlas, c, font) for c in str]
        end
        scale = const_lift(main, relative_scale) do str, s
            return Vec2f0[glyph_scale!(atlas, c, font, s) for c in str]
        end
    end
    delete!(data, :font)
    return _default((DISTANCEFIELD, position), s, data)
end
