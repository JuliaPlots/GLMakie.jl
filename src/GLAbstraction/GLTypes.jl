...
struct Shader
    ...
    function Shader(name, source, typ, id)
        return new(name, source, typ, id, current_context())
    end
end
...
mutable struct GLProgram
    ...
    context::GLContext
    function GLProgram(id::GLuint, shader::Vector{Shader}, nametype::Dict{Symbol,GLenum},
                       uniformloc::Dict{Symbol,Tuple})
...
function julia2glenum(::Type{T}) where {T}
    return error("Type: $T not supported as opengl number datatype")
end
...
mutable struct GLVertexArray{T}
    ...
    function GLVertexArray{T}(program, id, bufferlength, buffers, indices) where {T}
        return new(program, id, bufferlength, buffers, indices, current_context())
    end
end
    ...
    for (name, buffer) in bufferdict
        ...
        elseif Symbol(name) == :indices
            ...
            if len != length(buffer)
                ...
                error("Buffer $attribute does not have the same length as the other buffers." * bufferlengths)
            end
            bind(buffer)
            attribLocation = get_attribute_location(program.id, attribute)
            (attribLocation == -1) && continue
            glVertexAttribPointer(attribLocation, cardinality(buffer), julia2glenum(eltype(buffer)), GL_FALSE,
                                  0, C_NULL)
...
function GLVertexArray(program::GLProgram, buffers::Buffer, triangles::AbstractVector{<:GLTriangleFace})
    ...
    for property_name in propertynames(buffers)
        ...
        glVertexAttribPointer(attribLocation, cardinality(buffer), julia2glenum(eltype(buffer)), GL_FALSE, 0,
                              C_NULL)
        glEnableVertexAttribArray(attribLocation)
        buffers[attribute] = buffer
    end
...
mutable struct RenderObject{Pre}
    ...
    postrenderfunction::Any
    id::GLushort
    boundingbox::Any          # workaround for having lazy boundingbox queries, while not using multiple dispatch for boundingbox function (No type hierarchy for RenderObjects)
    function RenderObject{Pre}(main, uniforms::Dict{Symbol,Any}, vertexarray::GLVertexArray,
                               prerenderfunctions, postrenderfunctions, boundingbox) where {Pre}
        RENDER_OBJECT_ID_COUNTER[] += one(GLushort)
        return new(main, uniforms, vertexarray, prerenderfunctions, postrenderfunctions,
                   RENDER_OBJECT_ID_COUNTER[], boundingbox)
    end
end

function RenderObject(data::Dict{Symbol,Any}, program, pre::Pre, post,
                      bbs=Node(FRect3D(Vec3f0(0), Vec3f0(1))), main=nothing) where {Pre}
    ...
    robj = RenderObject{Pre}(main, data, vertexarray, pre, post, bbs)
