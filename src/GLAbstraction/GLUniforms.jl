...
const NATIVE_TYPES = Union{StaticArray,GLSL_COMPATIBLE_NUMBER_TYPES...,ZeroIndex{GLint},ZeroIndex{GLuint},
...
function uniformfunc(typ::DataType, dims::Tuple{Int,Int})
    M, N = dims
    return Symbol(string("glUniformMatrix", M == N ? "$M" : "$(M)x$(N)", opengl_postfix(typ)))
end
...
function gluniform(location::Integer, target::Integer, t::GPUVector)
    return gluniform(GLint(location), GLint(target), t.buffer)
end
function gluniform(location::Integer, target::Integer, t::Node)
    return gluniform(GLint(location), GLint(target), to_value(t))
end
function gluniform(location::Integer, target::Integer, t::TextureBuffer)
    return gluniform(GLint(location), GLint(target), t.texture)
end
...
function glsl_typename(t::Type{T}) where {T<:Union{StaticVector,Colorant}}
...
function glsl_typename(t::Type{T}) where {T<:SMatrix}
    M, N = size(t)
    return string(opengl_prefix(eltype(t)), "mat", M == N ? M : string(M, "x", N))
end
toglsltype_string(t::Node) = toglsltype_string(to_value(t))
function toglsltype_string(x::T) where {T<:Union{Real,StaticArray,Texture,Colorant,TextureBuffer,Nothing}}
...
function glsl_variable_access(keystring, t::Any)
    return error("no glsl variable calculation available for : ", keystring, " of type ", typeof(t))
end

function uniform_name_type(program::GLuint)
    uniformLength = glGetProgramiv(program, GL_ACTIVE_UNIFORMS)
    return Dict{Symbol,GLenum}(ntuple(uniformLength) do i # take size and name
                                   return name, typ = glGetActiveUniform(program, i - 1)
                               end)
end
function attribute_name_type(program::GLuint)
    uniformLength = glGetProgramiv(program, GL_ACTIVE_ATTRIBUTES)
    return Dict{Symbol,GLenum}(ntuple(uniformLength) do i
                                   return name, typ = glGetActiveAttrib(program, i - 1)
                               end)
end
function istexturesampler(typ::GLenum)
    return (typ == GL_SAMPLER_BUFFER ||
            typ == GL_INT_SAMPLER_BUFFER ||
            typ == GL_UNSIGNED_INT_SAMPLER_BUFFER ||
            typ == GL_IMAGE_2D ||
            typ == GL_SAMPLER_1D ||
            typ == GL_SAMPLER_2D ||
            typ == GL_SAMPLER_3D ||
            typ == GL_UNSIGNED_INT_SAMPLER_1D ||
            typ == GL_UNSIGNED_INT_SAMPLER_2D ||
            typ == GL_UNSIGNED_INT_SAMPLER_3D ||
            typ == GL_INT_SAMPLER_1D ||
            typ == GL_INT_SAMPLER_2D ||
            typ == GL_INT_SAMPLER_3D ||
            typ == GL_SAMPLER_1D_ARRAY ||
            typ == GL_SAMPLER_2D_ARRAY ||
            typ == GL_UNSIGNED_INT_SAMPLER_1D_ARRAY ||
            typ == GL_UNSIGNED_INT_SAMPLER_2D_ARRAY ||
            typ == GL_INT_SAMPLER_1D_ARRAY ||
            typ == GL_INT_SAMPLER_2D_ARRAY)
end
...
function isa_gl_struct(x::T) where {T}
    ...
    fnames = fieldnames(T)
    return !isempty(fnames) &&
           all(name -> isconcretetype(fieldtype(T, name)) && isbits(getfield(x, name)), fnames)
end
function gl_convert_struct(x::T, uniform_name::Symbol) where {T}
    if isa_gl_struct(x)
        return Dict{Symbol,Any}(map(fieldnames(x)) do name
                                    return (Symbol("$uniform_name.$name") => gl_convert(getfield(x, name)))
                                end)
    else
...
function lift_convert(a::ShaderAbstractions.Sampler, T, N)
    return ShaderAbstractions.Sampler(lift(x -> convert(Array{T,N}, x.data), a), minfilter=a[].minfilter,
                                      magfilter=a[].magfilter, x_repeat=a[].repeat[1],
                                      y_repeat=a[].repeat[min(2, N)], z_repeat=a[].repeat[min(3, N)],
