...
# Mesh
display(mesh(Sphere(Point3f0(0), 1.0f0)))
mesh(Sphere(Point3f0(0), 1.0f0), color=:red, ambient=Vec3f0(0.9))

tocolor(x) = RGBf0(x...)
positions = Observable(decompose(Point3f0, Sphere(Point3f0(0), 1.0f0)))
triangles = Observable(decompose(GLTriangleFace, Sphere(Point3f0(0), 1.0f0)))
uv = Observable(GeometryBasics.decompose_uv(Sphere(Point3f0(0), 1.0f0)))
...
p = ShaderAbstractions.Program(OGLContext(), read(assetpath("shader", "mesh.vert"), String),
                               read(assetpath("shader", "mesh.frag"), String), instance;)

println(p.vertex_source)

uniforms = Dict{Symbol,Any}(:texturecoordinates => Vec2f0(0), :image => nothing)
...
scren = display(mesh(vertexcolor_mesh, show_axis=false))
...
scren = display(mesh(texture_mesh, show_axis=false))
