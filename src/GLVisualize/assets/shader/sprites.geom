...
void emit_vertex(vec4 vertex, vec2 uv)
{
    gl_Position       = vertex;
    f_uv              = uv;
    f_uv_offset       = g_uv_offset_width[0];
...
void main(void)
{
    ...
    // zero at a radius of ALIASING_CONST pixels in the viewport coordinates
    ...
    // just do something which is cheap-ish and works for non-rotated sprites.
    // For rotated sprites it will help, but is not technically correct.
    //
    // TODO: Think about aa_clip normalization
    vec4 aa_clip = trans*vec4(1,1,0,0);
    vec2 aa_viewport = 0.5*resolution*aa_clip.xy / datapoint.w;
    float aa_buf = ALIASING_CONST/length(aa_viewport);
