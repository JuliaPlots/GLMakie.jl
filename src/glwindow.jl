...
rcpframe(x) = 1.0f0 ./ Vec2f0(x[1], x[2])
...
"""
function postprocess(color, position, normal_occlusion, ssao_noise, objectid, color_luma, framebuffer_size)
    ...
    kernel = map(1:N_samples) do i
        ...
        return v = Vec3f0(scale * rand() * n)
    end

    # compute occlusion
    shader1 = LazyShader(loadshader("postprocessing/fullscreen.vert"), loadshader("postprocessing/SSAO.frag"),
                         view=Dict("N_samples" => "$N_samples"))
    data1 = Dict{Symbol,Any}(:position_buffer => position, :normal_occlusion_buffer => normal_occlusion,
                             :kernel => kernel, :noise => ssao_noise,
                             :noise_scale => map(s -> Vec2f0(s ./ 4.0), framebuffer_size),
                             :projection => Node(Mat4f0(I)), :bias => Node(0.025f0), :radius => Node(0.5f0))
    ...
    shader2 = LazyShader(loadshader("postprocessing/fullscreen.vert"),
                         loadshader("postprocessing/SSAO_blur.frag"))
    data2 = Dict{Symbol,Any}(:normal_occlusion => normal_occlusion, :color_texture => color, :ids => objectid,
                             :inv_texel_size => map(t -> Vec2f0(1.0f0 / t[1], 1.0f0 / t[2]), framebuffer_size),
    ...
    shader3 = LazyShader(loadshader("postprocessing/fullscreen.vert"),
                         loadshader("postprocessing/postprocess.frag"))
    data3 = Dict{Symbol,Any}(:color_texture => color)
    ...
    # perform FXAA
    shader4 = LazyShader(loadshader("postprocessing/fullscreen.vert"), loadshader("postprocessing/fxaa.frag"))
    data4 = Dict{Symbol,Any}(:color_texture => color_luma, :RCPFrame => lift(rcpframe, framebuffer_size))
    ...
    # draw color buffer
    shader5 = LazyShader(loadshader("postprocessing/fullscreen.vert"), loadshader("postprocessing/copy.frag"))
    data5 = Dict{Symbol,Any}(:color_texture => color)
...
function GLFramebuffer(fb_size::NTuple{2,Int})
    ...
    depth_buffer = Texture(Ptr{GLAbstraction.DepthStencil_24_8}(C_NULL), fb_size, minfilter=:nearest,
                           x_repeat=:clamp_to_edge, internalformat=GL_DEPTH24_STENCIL8,
                           format=GL_DEPTH_STENCIL)

    ssao_noise = Texture([normalize(Vec2f0(2.0rand(2) .- 1.0)) for _ in 1:4, __ in 1:4], minfilter=:nearest,
                         x_repeat=:repeat)
    ...
    p = postprocess(color_buffer, position_buffer, normal_occlusion_buffer, ssao_noise, #occlusion,
                    objectid_buffer, color_luma, fb_size_node)

    return GLFramebuffer(fb_size_node, (render_framebuffer, color_luma_framebuffer), color_buffer,
                         objectid_buffer, depth_buffer, position_buffer, normal_occlusion_buffer, ssao_noise,# occlusion,
                         color_luma, p)
end
...
function MonitorProperties(monitor::GLFW.Monitor)
    ...
    return MonitorProperties(name, isprimary, position, physicalsize, videomode, videomode_supported, dpi,
                             monitor)
end
